...
export function TaskEditor({
  ...
  const [occurrenceTitle, setOccurrenceTitle] = useState(task.title);
  const [occurrenceNotes, setOccurrenceNotes] = useState(task.notes);
  const [occurrenceStartDate, setOccurrenceStartDate] = useState(task.startDate);
  const [occurrenceEndDate, setOccurrenceEndDate] = useState(task.endDate);
  const [seriesTitle, setSeriesTitle] = useState(series?.title ?? task.title);
  const [seriesNotes, setSeriesNotes] = useState(series?.notes ?? task.notes);
  ...
  const seriesWide = Boolean(
    series && allowSeriesActions && scope === "series",
  );
  const seriesDate = task.occurrenceDate ?? task.startDate;
  const title = seriesWide ? seriesTitle : occurrenceTitle;
  const notes = seriesWide ? seriesNotes : occurrenceNotes;
  const startDate = seriesWide ? seriesDate : occurrenceStartDate;
  const endDate = seriesWide ? seriesDate : occurrenceEndDate;
  const setTitle = seriesWide ? setSeriesTitle : setOccurrenceTitle;
  const setNotes = seriesWide ? setSeriesNotes : setOccurrenceNotes;
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!seriesWide && endDate < startDate) {
      setFormError("截止日期不能早于开始日期");
      return;
    }

    const editingRecurrence = !series || seriesWide;
    if (editingRecurrence && recurrenceKind !== "none" && startDate !== endDate) {
        ...
        <Modal.Container placement="center" scroll="inside" size="md">
          <Modal.Dialog className="task-dialog" aria-labelledby="task-dialog-title">
            ...
            <Modal.Body className="task-dialog__body">
              {loading ? <p role="status">正在同步重复规则…</p> : null}
              {loadError ? (
                <div className="planner-status planner-status--error" role="alert">
                  <p>{loadError}</p>
                  <Button type="button" variant="ghost" size="sm" onPress={onRetryLoad}>
                    重试读取重复规则
                  </Button>
                </div>
              ) : null}
                  ...
                  <TextField className="editor-field">
                    ...
                    <Input
                      ...
                      onChange={(event) => {
                        ...
                        if (occurrenceEndDate < value || task.seriesId) {
                  ...
                  <TextField className="editor-field">
                    ...
                    <Input
                      ...
                      onChange={(event) => {
                        ...
                        setOccurrenceEndDate(event.target.value);
                        setFormError(null);
                      }}
                    />
                  </TextField>
            ...
            <Modal.Footer className="task-dialog__actions">
              <div>
                ...
                {series && allowSeriesActions && onStopRecurrence ? (
                  <Button
                    variant="ghost"
                    type="button"
                    isDisabled={busy}
