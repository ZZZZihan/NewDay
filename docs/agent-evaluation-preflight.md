...
2026-09-21 在 COL-39 候选 `95e1be8b3ac5755f70579eedc6bd098d9dbf90a1` 上首次预检：52 个场景均能构建符合当前 schema 的生产快照。独立复核后补充任务时间线检查；保留集有三项不能无损进入后续真实试验：

| 场景 | 缺口 | 启动真实试验前的决定 |
| --- | --- | --- |
| H-C08 | 取样时间是 `2026-09-08T02:30Z`，但两个任务的创建和更新时间是当天 `08:00Z`。当前存储无法重建取样前的任务状态。 | 冻结修订版场景与清单摘要，明确两个任务在取样前已存在，并重新核对当地日期行为；原版保留作历史证据。 |
