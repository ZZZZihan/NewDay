...
- 用户在 Notion 授权页取消时，Worker 在回调中标记该 state 为取消，本机清除对应 verifier。单独调用本机 `/oauth/cancel` 只删除本机 verifier；Worker 会话到期后自行失效。断开某个已连接工作区会删除该工作区的本机凭据，并在领取时拒绝断开前发起的同工作区授权；其他工作区的待领取授权继续有效。若回调错误、过期或本机进程停止，重新开始授权；不要把未领取的会话算作已连接。
