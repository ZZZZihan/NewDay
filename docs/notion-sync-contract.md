...
规则的来源仅限于连接建立后从 Notion Rules 表读取的页面；已有本地重复规则仍按本地方式编辑和生成。Rules 和 Tasks 各自整表扫描成功后按表提交水位；Rules 已提交而 Tasks 失败时，新规则来源与既有实例的例外标记可以保留，但规则映射保持“生成边界待核对”，不生成新实例。只有后续 Tasks 完整扫描成功，才在同一 SQLite 事务中核对本轮读到的旧规则实例、确定生成边界、生成缺少的窗口实例、固定发生键并保存映射与待发送操作。Tasks 事务失败则这些任务、映射、操作及其成功水位一起回滚。普通清单读取和 Agent 快照只生成本地规则的实例，不能绕过扫描事务创建尚无映射的 Notion 实例。用户时区缺失、扫描不完整或存在未解决的发送结果时暂停生成，保留既有实例。

Notion 修改规则时，所有已经生成的实例都固定为显式例外：已完成历史、已发送的未来页面及尚未发送但已落库的实例保留原有标题、计划日、完成状态和发生键。规则映射记住这些实例中最晚的原始发生日；新版规则只在该日之后生成，避免把已生成的月末 31 日实例与改规则后同月 30 日实例同时创建。停用或归档规则也保留这些实例，仅停止继续生成；重新启用仍遵守已有生成边界。单个实例在 Notion 改期或改名，只修改该实例并保留原始发生键，不回写规则。NewDay 不自动删除或重建远端实例页面。

服务停机后从恢复当天重新建立今天至今天＋31 天的窗口；已生成的历史和未来实例按稳定键复用。超出既有窗口且停机期间从未生成的历史日期不补造实例，也不推断完成记录。此政策应随真实工作区验收和版本说明呈现给用户。
...
只读扫描的 429/529 遵守 `Retry-After` 秒数，并在再次失败时做有上限的退避和抖动；写前预读的 429/529 持久记录截止时间，截止前不允许恢复，之后由人触发下一次尝试。401/403、schema 错误、404 与可重试网络/服务错误分开。只有授权有效、API 运行、固定测试规模且无持续限流时才验收 5 分钟内反映变更。API 关闭、备份恢复和暂停状态明确告知用户。
...
COL-32 的交付范围是本契约、字段表、故障矩阵、API/SDK 文档核对与合成 fixture；这些文档经审阅并完成一致性检查后可作为 T1 契约交付。真实隔离 Notion 工作区的结构与关系读回归 T3 验收；条件写入能力、日期与回收站响应及实际冲突行为归 T5/T6 验收。T7 的离线故障测试采用上面的已生成实例冻结政策；真实工作区的运行行为仍须独立验收。上述运行行为未验收前，不可把整条 COL-31 标为完成。
