...
规则的来源仅限于连接建立后从 Notion Rules 表读取的页面；已有本地重复规则仍按本地方式编辑和生成。规则、任务两个数据源完成扫描后，NewDay 才在同一 SQLite 事务中生成缺少的窗口实例、固定其发生键、保存映射及待发送操作。普通清单读取和 Agent 快照只生成本地规则的实例，不能绕过扫描事务创建尚无映射的 Notion 实例。用户时区缺失、扫描不完整或存在未解决的发送结果时暂停生成，保留既有实例。
