...
用户可以针对一个明确 issue、批次和仓库授予持续交付授权，并把授权范围记录在对应 Linear issue、PR 或验收报告中。授权明确覆盖时，执行者可连续完成基线核对、复现、实现、测试、独立审查、修复、回归、原子提交、推送、PR 更新、门禁配置与回读、正常合并、`main` 验证和状态同步，不因同一范围内的重试或审查修复反复索要批准。

持续授权不扩大产品范围，也不覆盖平台或组织强制规则。强推受保护分支、关闭质量门禁、伪造审批或验收、使用未授权凭据或个人数据、生产部署、付费调用及不可逆外部写入，仍须由授权原文明确覆盖且满足上级规则。认证或外部权限确实缺失时，记录精确错误和最小恢复动作，并继续完成不依赖该权限的工作；测试失败进入修复循环，而不是自动把整个批次标成 blocked。只有最终候选的必需检查和独立审查都满足后，才能使用持续授权完成合并。
