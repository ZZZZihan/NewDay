...
| 架构与验证 | 当前迁移工作区的 `docs/architecture.md`，随 COL-22 收录 |
| Agent 阶段与既有证据 | 当前迁移工作区的 `docs/agent-development-status.md`，随 COL-22 收录 |

这两份迁移文档尚未进入本次流程配置提交。仅检出本次提交时，先从 COL-22 获取其交付状态；不要将本地未提交文件当成已经随 Git 发布的内容。
...
记录准确的命令、退出结果、候选 SHA，以及未运行的必要检查及原因。COL-22 中的 HTTP/SQLite 重构要求浏览器测试使用隔离端口 `3100/3002` 与临时 SQLite，该测试配置随 COL-22 另行提交。仅检出本次流程配置提交时，不能假定已有该隔离机制；执行替换导入场景前须核实候选版本的实际测试配置。现有测试记录只证明其记录的版本与范围；scripted 或 smoke 结果不能替代 G3/G4。
...
2026-09-08 初始化时，当前分支为 `feature/simple-daily-planner`，HEAD 为 `f7d8ccf6c7d36f3a7284fa87e082de18df0122b7`。原工作区有尚未提交的架构拆分与 Agent 实现；另有本地集成快照 `888d915e29f6b43b1f4a3acc130ea1e2ce0e2484`，其后工作区又有变化。COL-22 负责将这些状态核对为可审查的 Git 交付，本次流程初始化保留原分支、HEAD、暂存区及既有改动。
