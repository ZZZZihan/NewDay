...
type LegacyRecurrenceSeriesRecord = {
  id: string;
  logicalSeriesId?: string;
...
class NewDayDatabase extends Dexie {
  ...
  constructor(name: string) {
    ...
    this.version(5)
      .stores({
        tasks:
          "id, startDate, endDate, status, completedOn, updatedAt, seriesId, logicalSeriesId, occurrenceDate, &occurrenceKey",
        recurrenceSeries:
          "id, logicalSeriesId, startDate, effectiveEndDate, updatedAt, [logicalSeriesId+startDate]",
        focusRecords: "id, date, taskId, focusedAt, &[date+taskId]",
      })
      .upgrade(async (transaction) => {
        await Promise.all([
          transaction
            ...
            .modify((task) => {
              if (task.seriesId) {
                task.logicalSeriesId = task.seriesId;
              }
            }),
          transaction
            .table<LegacyRecurrenceSeriesRecord, string>("recurrenceSeries")
            .toCollection()
            .modify((series) => {
              series.logicalSeriesId = series.id;
...
export class DexiePlannerStore implements PlannerArchiveStore {
  ...
  async listRecurrenceSeriesByLogicalSeriesId(logicalSeriesId: string) {
    const series = await this.database.recurrenceSeries
      .where("logicalSeriesId")
      .equals(logicalSeriesId)
      .toArray();
