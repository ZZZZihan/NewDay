...
import { recurrenceOccurrenceKey } from "@newday/core/domain/planner-recurrence";
...
describe("ensureRecurrenceOccurrences", () => {
  ...
  it("materializes adjacent rule segments without gaps or duplicate identities", async () => {
    const store = new MemoryPlannerStore();
    await store.putRecurrenceSeries(
      series({
        id: "series-prefix",
    ...
    expect(await store.listAllTasks()).toEqual([
      ...
      expect.objectContaining({
        ...
        occurrenceKey: "series-1:2026-09-02",
      }),
      expect.objectContaining({
        seriesId: "series-tail",
      ...
      expect.objectContaining({
        ...
        occurrenceKey: "series-1:2026-09-04",
      }),
    ]);
  });
