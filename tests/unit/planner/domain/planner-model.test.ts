...
describe("recurrenceSeriesSchema", () => {
  ...
  it("accepts an inclusive effective boundary and rejects one before the segment", () => {
    expect(
      recurrenceSeriesSchema.safeParse({
        ...SERIES,
        effectiveEndDate: SERIES.startDate,
      }).success,
    ).toBe(true);
    expect(
      recurrenceSeriesSchema.safeParse({
  ...
  it("rejects an explicit recurrence end before the segment boundary", () => {
    expect(
      recurrenceSeriesSchema.safeParse({
        ...SERIES,
