...
describe("MemoryPlannerStore", () => {
  ...
  it("lists a logical recurrence chain in deterministic effective order", async () => {
    const store = new MemoryPlannerStore();
    const tail = recurrenceSeries({
      id: "series-tail",
      logicalSeriesId: "logical-series",
      startDate: "2026-09-03",
    });
    const prefix = recurrenceSeries({
      id: "series-prefix",
      logicalSeriesId: "logical-series",
    ...
    await store.putRecurrenceSeries(
      recurrenceSeries({ id: "other-series", logicalSeriesId: "other-series" }),
    );

    expect(
      await store.listRecurrenceSeriesByLogicalSeriesId("logical-series"),
    ).toEqual([prefix, tail]);
  });
